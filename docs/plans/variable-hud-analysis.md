...
| # | 议题 | 锁定结论 |
|---|------|----------|
| 1 | WS / `room_id` 等契约 | **严格按原 Spec**（`variable_update` payload 与行为），不做额外变更或另起策略 |
| 2 | no-op（`changed: false`）是否广播 | **不广播** |
| 3 | Activity 同屏 Toast / Window | **层级下降一级**；优先保证**房间内容**可读，标签/chrome 不得压过房间正文 |
| 4 | PR 拆分 | **前端 / 后端分两个 PR**；先做前端（`variable-viz` + `VariableHudPanel` + 布局），WS/store 广播随后端 PR |

### 层级约定（前端）

```
房间内容面（消息 / Status Bar）  >  chrome 标签（顶栏状态字）  >  Toast / 瞬时 Window
```

CSS 变量：`--z-room-surface` > `--z-chrome-label` > `--z-toast`。4.3 特效 Toast 必须使用 `--z-toast`，且 `pointer-events-none`。
...
### Slice A — 前端 PR（本轮，不依赖新后端）

1. `frontend/lib/variable-viz.ts` + 单测（Spec §5.4 / §8.1）  
2. `VariableHudPanel`（Spec §4 / §5.1–5.2）  
3. `chat-layout` 三栏；`resolveHudDisplays([], roomVariables)`  
4. 层级：房间内容优先于 chrome 标签 / Toast  
5. 验收：侧栏 `/setvar` 或 CRUD 后右侧出现条（仍靠现有 `loadVariables`）

### Slice B — 后端 PR（随后）

6. shared `variable_update`（**严格按 Spec**）  
7. store notifier + room-hub；**no-op 不广播**  
8. 前端 `use-websocket` + merge `roomVariables`  
9. 验收：Agent `inc_variable` 无手动刷新

### 其后

- **4.2** schema/API/bootstrap（含 `room_variables` 导入）  
- **4.3** 特效（Toast 使用 `--z-toast`）  
- **4.4** 窄屏 + 侧栏色阶 + E2E  
...
| 2026-07-13 | 评审锁定：跟 Spec、no-op 不广播、同屏层级、前后端分 PR；启动前端 Slice A |
